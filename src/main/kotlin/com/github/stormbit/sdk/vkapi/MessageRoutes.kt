...
import com.github.stormbit.sdk.events.chat.ChatScreenshotEvent
...
abstract class MessageRoute<E : MessageEvent>(
    ...
    @Suppress("unused")
    open fun handle(block: suspend ApiContext.(E) -> Unit) {
        if (handler != null)
...
open class TypedMessageRoute<E : MessageEvent>(
    ...
    phrases: List<String>
) : MessageRoute<E>(context, parent, senderType, attachmentType, null, phrases) {

    suspend fun onCommand(vararg words: String, block: suspend DefaultMessageRoute.() -> Unit) {
        DefaultMessageRoute(context, this, words.toList())
    ...
    suspend inline fun <reified T : AttachmentEvent> onMessageWith(noinline block: suspend TypedMessageRoute<T>.() -> Unit) {
        ...
        TypedMessageRoute<T>(context, this, senderType, attachmentType, phrases)
            .also { children.add(it) }.block()
    }
}
...
class DefaultMessageRoute(
    ...
    parent: MessageRoute<*>?,
    phrases: List<String>
) : MessageRoute<MessageEvent>(context, parent, null, null, null, phrases) {
...
) : MessageRoute<E>(context, parent, null, null, serviceActionType, emptyList())
